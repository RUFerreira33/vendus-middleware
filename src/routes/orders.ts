...
ordersRouter.get("/", asyncHandler(async (req, res) => {
  const params = pickQuery(req, [
    "store_id","register_id","client_id","client_fiscal_id","client_country",
    "subtype","since","until","q","external_reference","status","view","mode",
    "per_page","page"
  ]);
...
ordersRouter.get("/:id", asyncHandler(async (req, res) => {
  const params = pickQuery(req, [
    "mode","copies","output","output_template_id","output_version",
    "return_qrcode","download","force_template","register_id"
  ]);
  const qs = toQueryString(params);
